...
public class KosmXJotaroOraOra extends KosmXWindupAttackHandler {

    public KosmXJotaroOraOra(ResourceLocation id){
        super(id);
    }
    ...
    private static final ResourceLocation ORA_ORA = new ResourceLocation(JojoMod.MOD_ID, "jotaro_ora_ora");

    @Override
    public boolean setWindupAnim(PlayerEntity player) {
        return setAnimFromName(player, ORA_ORA, anim -> new ChargedAttackAnimPlayer(anim).windupStopsAt(anim.returnToTick));
    }

    @Override
    public boolean setAttackAnim(PlayerEntity player) {
        return setToSwingTick(player, -1, ORA_ORA);
    }
