...
public class KosmXDioMudaMuda extends KosmXWindupAttackHandler {

    public KosmXDioMudaMuda(ResourceLocation id){
        super(id);
    }
    ...
    private static final ResourceLocation MUDA_MUDA = new ResourceLocation(JojoMod.MOD_ID, "dio_muda_muda");

    @Override
    public boolean setWindupAnim(PlayerEntity player) {
        return setAnimFromName(player, MUDA_MUDA, anim -> new ChargedAttackAnimPlayer(anim).windupStopsAt(anim.returnToTick));
    }

    @Override
    public boolean setAttackAnim(PlayerEntity player) {
        return setToSwingTick(player, -1, MUDA_MUDA);
    }
