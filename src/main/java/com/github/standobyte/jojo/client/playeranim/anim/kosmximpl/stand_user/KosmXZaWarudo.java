package com.github.standobyte.jojo.client.playeranim.anim.kosmximpl.stand_user;
...
public class KosmXZaWarudo extends KosmXWindupAttackHandler {

    public KosmXZaWarudo(ResourceLocation id){
        super(id);
    }
    ...
    private static final ResourceLocation ZA_WARUDO = new ResourceLocation(JojoMod.MOD_ID, "za_warudo");

    @Override
    public boolean setWindupAnim(PlayerEntity player) {
        return setAnimFromName(player, ZA_WARUDO, anim -> new ChargedAttackAnimPlayer(anim).windupStopsAt(anim.returnToTick));
    }

    @Override
    public boolean setAttackAnim(PlayerEntity player) {
        return setToSwingTick(player, -1, ZA_WARUDO);
    }
